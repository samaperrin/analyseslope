...
#test
#slope_analysis_fish <- gradient_analysis_perch
#your_upstream_slopes <- upstream_slopes
#species <- "Pike"
#slope_parameters <- c("gradient_mean","gradient_third_quart","gradient_max","gradient_max_max")
#DoS <- "slope"
#plot_dispersal_curves(your_upstream_slopes, slope_analysis_fish, species, slope_parameters,DoS = "slope")

plot_dispersal_curves <- function(your_upstream_slopes, slope_analysis_fish, species, slope_parameters,DoS){
  ...
  if (DoS=="slope" & condition2 == 2) {
    ...
    x.limit <- max(your_upstream_slopes[,slope_parameters])/100
    ...
    if (DoS=="slope" & condition2 == 1){
      ...
      if (DoS=="distance" & condition2 == 1) {
        ...
        for(i in 1:length(slope_parameters)){
          ...
          avg.slope <- log(median(upstream_slopes[,slope_parameters[i]]))
